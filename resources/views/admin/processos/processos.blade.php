...
@section('content')
    ...
    <div class="main container-fluid">
            ...
            @if ($user->eh_admin == '1')
                ...
                <div>
                    <a href="" data-toggle="modal" data-target="#criarprocesso" type="submit"
                        class="btn btn-success btn-flat btn-addon m-b-10 m-l-2 mt-4 mr-5"><i class="ti-plus"></i>Add
                        Processo</a>
                </div>
            @endif
        </div>
        ...
        <div>
            <button class="btn btn-primary mb-10 ml-3" type="button" data-toggle="collapse" data-target="#tabela_status"
                aria-expanded="false" aria-controls="tabela_status">Mostrar status de etapas</button>
                        ...
                        <div class="col-md-12">
                            <div class="table-overflow">
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            <th class="text-primary">Disciplinas</th>
                                            @foreach ($todas_etapas as $et)
                                                <th style="text-align: center; line-height: 15px">{{ $et->nome }}</th>
                                            @endforeach
                                        </tr>
                                    </thead>
                                    <tbody>
                                                    ...
                                                    @foreach ($todos_processos as $proc)
                                                        @if ($proc->etapa_id == $t_etapas->id && $proc->disciplina_id == $disciplina->id)
                                                            ...
                                                            <td style="text-align: center">
                                                                @if ($proc->status == 1)
                                                                    <i
                                                                        class="ti-check-box btn btn-success btn-sm m-b-10 m-l-5"></i>
                                                                @else
                                                                    <i
                                                                        class="ti-close btn btn-danger btn-sm m-b-10 m-l-5"></i>
                                                                @endif
                                                            </td>
                                                        @break
                                                    @endif
            ...
            <table id="processos" class="table table-striped" style="width:100%">
                ...
                <tbody>
                    @foreach ($processos as $processo)
                        <tr>
                            <td class="badge badge-danger w-100 h-25" style="font-size: 15px">
            ...
            <div class="modal-body">
                <form action="{{ route('processos.store') }}" method="POST" class="forms-sample">
                    ...
                    <div class="row">
                        ...
                        <div class="d-none"><input type="text" name="projeto_id"
                                value="{{ $etapa->projeto_id }}"></div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="date" class="form-control" id="data_entrega_autor"
                                name="data_entrega_autor" value="">
                        </div>
